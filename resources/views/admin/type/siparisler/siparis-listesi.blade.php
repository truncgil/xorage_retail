...
<a name="liste"></a>
    <form action="#liste" method="get">
                    <div class="row">
                       <div class="col-auto">
                           {{e2("SİPARİŞ NUMARASI")}}: 
                            <input type="text" name="siparis_id" value="{{get("siparis_id")}}" id="" class="form-control">
                       </div>
                        <div class="col-auto">
                            {{e2("ÜRÜN GRUBU")}} : <br>
                            <select name="urun" id="" class="form-control select2">
                                ...
                                <?php if($m->title!="")  { 
                                  ?>
                                 <option value="{{$m->title}}" <?php if(getesit("urun",$m->title)) echo "selected"; ?>>{{$m->title}}</option> 
                                 <?php } ?>
                                <?php } ?>
                            </select>
                        </div>
                        ...
                        <div class="col-auto">
                            ...
                            <select name="durum" id="" class="form-control">
                                ...
                                <?php foreach(siparis_durumlari() AS $d)  { 
                                  ?>
                                 <option value="{{$d}}" <?php if(getesit("durum",$d)) echo "selected"; ?>>{{$d}}</option> 
                                 <?php } ?>
                            </select>
                        </div>
                        <div class="col-md-6">
    ...
    {{_col()}}
{{col("col-md-12","Oluşturulan Siparişler")}}
    ...
    <div class="table-responsive">
        <table  class="table" id="excel">
            <tr>
                <td>{{e2("Görsel")}}</td>
                <td>{{e2("ID")}}</td>
                <td>{{e2("Sipariş Numarası")}}</td>
                <td>{{e2("Sipariş Ürün Grubu Adı")}}</td>
                <td>{{e2("Sipariş İçeriği")}}</td>
                <td>{{e2("Sipariş Miktarı")}}</td>
                <td>{{e2("Tarih")}}</td>
                <td>{{e2("Açıklama")}}</td>
                <td>{{e2("Durum")}}</td>
                <td>{{e2("İşlem")}}</td>
            ...
            foreach($siparisler AS $s)  { 
             ...
             <tr>
                 ...
                 <td>
                     ...
                     <form action="" id="form{{$s->id}}" method="post">
                         @csrf
                    <table class="table table-striped table-bordered">
                        ...
                        try {
                            foreach($j['urun'] AS $urun)  { 
                               ...
                               ?>
                              <tr>
                                  <td>{{$bu_urun->title}} {{$bu_urun->renk}}</td>
                                  <td>{{$j['qty'][$k]}}</td>
                                  <td><input type="checkbox" name="{{$k}}" <?php if(@$tamamlandi[$k]=="on") echo "checked"; ?>   class="tamamlandi" data-id="{{$s->id}}" id=""></td>
                              </tr>  
                                 <?php } ?> 
                                 <?php $k++; } ?>
                            <?php 
                    ...
                    <?php if($s->json2!="") {
                         ?>
                          <a class="btn btn-primary" title="{{e2("Bu siparişte değişiklikler yapıldı. Değişikliği görmek için tıklayınız")}}" data-toggle="collapse" href="#collapseExample{{$s->id}}" role="button" aria-expanded="false" aria-controls="collapseExample{{$s->id}}">
                            ...
                            <div class="card card-body">
                               ...
                               <table class="table table-striped table-bordered">
                                ...
                                <?php if($json2['html']!=$s->html)  { 
                                  ?>
                                 <strong>{{e2("Değiştirilen Açıklama")}}</strong> : {{$json2['html']}} 
                                 <?php } ?>
                            </div>
                        </div>
                 ...
                 <td>
                     <select name="durum" table="siparis_grubu" id="{{$s->id}}" class="form-control edit">
                         <option value="">Seçiniz</option>
                         <?php $durumlar = siparis_durumlari();
                         foreach($durumlar AS $d)  { 
                          
                          ?>
                          <option value="{{$d}}" <?php if($d==$s->durum) echo "selected"; ?>>{{$d}}</option> 
                          <?php } ?>
                     </select>
                 </td>
                 <td>
                     <a href="?siparis-duzenle={{$s->id}}" class="btn btn-success"><i class="fa fa-edit"></i></a>
                     <a href="?siparis-sil={{$s->id}}" teyit="{{e2("Bu siparişi silmek istediğinizden emin misiniz?")}}" class="btn btn-danger"><i class="fa fa-times"></i></a>
                 </td>
             </tr> 
             <?php } ?>
        </table>
